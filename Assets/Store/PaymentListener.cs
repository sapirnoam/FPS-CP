...
public class PaymentListener : MonoBehaviour {
	...
	private void transitionToSuccess() {
		...
		Debug.Log ("success");
        //Automatic Write command after payment accepted:

        //Write command:
        UseItem.Use(StoreActions.INSTANCE.PurchaseItemNameField.text);

        //Sound:
        AudioCash CashAudio = GameObject.Find("Cash").GetComponent<AudioCash>();
        CashAudio.TenTokenCash(StoreActions.INSTANCE.PurchaseItemNameField.text);
        CashAudio.FiftyTokenCash(StoreActions.INSTANCE.PurchaseItemNameField.text);
        CashAudio.HundredTokenCash(StoreActions.INSTANCE.PurchaseItemNameField.text);

        //Fild for a bounus tokens or something..
    }
